...
abstract class BasicFilenameDecryptionStrategy extends FilenameDecryptionStrategy {
	...
	BasicFilenameDecryptionStrategy(EncFSVolume volume, String volumePath, EncFSFilenameEncryptionAlgorithm algorithm) {
	...
	protected String decryptImpl(String fileName) throws EncFSCorruptDataException, EncFSChecksumException {
		...
		byte[] chainIv = EncFSCrypto.computeChainedIV(volume, volumePath, config);
		byte[] base256FileName = EncFSBase64.decodeEncfs(fileName.getBytes());
		byte[] macBytes = EncFSCrypto.getMacBytes(base256FileName);
		byte[] encFileName = ArrayUtil.copyOfRange(base256FileName, 2, base256FileName.length);
	...
	private void verifyDecryptionWorked(EncFSVolume volume, byte[] chainIv, byte[] base256FileName, byte[] decFileName) throws EncFSChecksumException {
		...
		byte[] expectedMac = ArrayUtil.copyOfRange(base256FileName, 0, 2);
