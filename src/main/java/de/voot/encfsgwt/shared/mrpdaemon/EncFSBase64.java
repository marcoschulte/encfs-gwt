...
import de.voot.encfsgwt.shared.jre.FilterInputStream;
import de.voot.encfsgwt.shared.jre.FilterOutputStream;
...
public final class EncFSBase64 {
	 ...
	 * @since 2.3.1
	 */
	private static byte[] encodeBytesToBytes(byte[] source, int off, int len, int options) throws IOException {
		...
		// Compress?
		if ((options & GZIP) != 0) {
			throw new UnsupportedOperationException("GZIP not implemented");
			/*
			 * ByteArrayOutputStream baos = null; GZIPOutputStream gzos = null;
			 * EncFSBase64.OutputStream b64os = null;
			 * 
			 * try { // GZip -> Base64 -> ByteArray baos = new
			 * ByteArrayOutputStream(); b64os = new
			 * EncFSBase64.OutputStream(baos, ENCODE | options); gzos = new
			 * GZIPOutputStream(b64os);
			 * 
			 * gzos.write(source, off, len); gzos.close(); } catch (IOException
			 * e) { // Catch it and then throw it immediately so that // the
			 * finally{} block is called for cleanup. throw e; } finally { try {
			 * gzos.close(); } catch (Exception e) { } try { b64os.close(); }
			 * catch (Exception e) { } try { baos.close(); } catch (Exception e)
			 * { } }
			 * 
	...
	private static int decode4to3(byte[] source, byte[] destination, int destOffset, int options) {
		...
		if (3 >= source.length) {
			throw new IllegalArgumentException("Source array with length " + source.length + " cannot have offset of 0 and still process four bytes.");
		}
		if (destOffset < 0 || destOffset + 2 >= destination.length) {
			throw new IllegalArgumentException("Destination array with length " + destination.length + " cannot have offset of " + destOffset
		...
		if (source[2] == EQUALS_SIGN) {
			...
			int outBuff = ((DECODABET[source[0]] & 0xFF) << 18) | ((DECODABET[source[1]] & 0xFF) << 12);
		...
		// Example: DkL=
			...
			int outBuff = ((DECODABET[source[0]] & 0xFF) << 18) | ((DECODABET[source[1]] & 0xFF) << 12) | ((DECODABET[source[2]] & 0xFF) << 6);
	...
	private static byte[] decode(byte[] source, int len, int options) throws IOException {
		...
		if (len > source.length) {
			throw new IllegalArgumentException("Source array with length " + source.length + " cannot have offset of 0 and process " + len + " bytes.");
		}
	...
	private static byte[] decode(String s, int options) throws IOException {
		...
		if ((bytes != null) && (bytes.length >= 4) && (!dontGunzip)) {
			...
			if (java.util.zip.GZIPInputStream.GZIP_MAGIC == head) {
				throw new UnsupportedOperationException("GZIP not implemented");
				/*
				 * ByteArrayInputStream bais = null; GZIPInputStream gzis =
				 * null; ByteArrayOutputStream baos = null; byte[] buffer = new
				 * byte[2048];
				 * 
				 * try { baos = new ByteArrayOutputStream(); bais = new
				 * ByteArrayInputStream(bytes); gzis = new
				 * GZIPInputStream(bais);
				 * 
				 * int length; while ((length = gzis.read(buffer)) >= 0) {
				 * baos.write(buffer, 0, length); }
				 * 
				 * // No error? Get new bytes. bytes = baos.toByteArray();
				 * 
				 * } catch (IOException e) { e.printStackTrace(); // Just return
				 * originally-decoded bytes } finally { try { baos.close(); }
				 * catch (Exception e) { } try { gzis.close(); } catch
				 * (Exception e) { } try { bais.close(); } catch (Exception e) {
				 * } }
				 */
			}
		}
	...
	static class InputStream extends FilterInputStream {
		 ...
		 * @since 1.3
		 */
		public InputStream(de.voot.encfsgwt.shared.jre.InputStream in) {
			this(in, DECODE);
		}
		 ...
		 * @since 2.0
		 */
		public InputStream(de.voot.encfsgwt.shared.jre.InputStream in, int options) {
	 ...
	 * @since 1.3
	 */
	static class OutputStream extends FilterOutputStream {
		 ...
		 * @since 1.3
		 */
		public OutputStream(de.voot.encfsgwt.shared.jre.OutputStream out) {
			this(out, ENCODE);
		}
		 ...
		 * @since 1.3
		 */
		public OutputStream(de.voot.encfsgwt.shared.jre.OutputStream out, int options) {
			super(out);
			this.breakLines = (options & DO_BREAK_LINES) != 0;
			this.encode = (options & ENCODE) != 0;
			...
			// Else, Decoding
				...
				if (decodabet[theByte & 0x7f] > WHITE_SPACE_ENC) {
					...
					if (position >= bufferLength) { // Enough to output.
						...
						int len = EncFSBase64.decode4to3(buffer, b4, 0, options);
						out.write(b4, 0, len);
						position = 0;
					}
