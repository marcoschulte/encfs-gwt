...
public class EncFSInputStream extends FilterInputStream {
	 ...
	 *            externalIVChaining)
	 */
	public EncFSInputStream(EncFSVolume volume, InputStream in, String volumePath) throws EncFSCorruptDataException, EncFSUnsupportedException {
		super(in);
		this.volume = volume;
		...
		if (config.isUseUniqueIV()) {
			...
			try {
				this.fileIv = StreamCrypto.streamDecrypt(volume, initIv, fileHeader);
	...
	public int read(byte[] output, int offset, int size) throws IOException {
		...
		while (bytesRead < size) {
			...
			bytesToCopy = Math.min(blockBuf.length - bufCursor, size - bytesRead);
		...
		if (bytesRead == blockSize) { // block decode
			...
			try {
				if (zeroBlock) {
					...
					blockBuf = BlockCrypto.blockDecrypt(volume, getBlockIV(), cipherBuf);
				}
			} catch (InvalidAlgorithmParameterException e) {
				e.printStackTrace();
			} catch (IllegalBlockSizeException e) {
			...
			try {
				blockBuf = StreamCrypto.streamDecrypt(volume, getBlockIV(), cipherBuf, 0, bytesRead);
			} catch (InvalidAlgorithmParameterException e) {
				e.printStackTrace();
			} catch (IllegalBlockSizeException e) {
				throw new EncFSCorruptDataException(e);
