...
import de.voot.encfsgwt.shared.crypto.Cipher;
import de.voot.encfsgwt.shared.crypto.Mac;
import de.voot.encfsgwt.shared.jre.FileNotFoundException;
...
public class EncFSVolume {
	...
	void readConfigAndInitVolume(final Callback<Void, Exception> callback) throws EncFSUnsupportedException, EncFSInvalidConfigException,
	EncFSCorruptDataException, EncFSInvalidPasswordException, IOException {

		byte[] keyData;
		try {
			keyData = VolumeKey.decryptVolumeKey(volumeConfig, passwordDerivedKeyData);
		...
		volumeKey = EncFSCrypto.newKey(ArrayUtil.copyOfRange(keyData, 0, keyLength));
		...
		getFile(ROOT_PATH, new Callback<EncFSFile, Exception>() {
			@Override
	...
	private byte[] copyIVdata(byte[] keyData, int keyLength) throws EncFSInvalidConfigException {
		...
		return ArrayUtil.copyOfRange(keyData, keyLength, keyLength + ivLength);
	}
	 ...
	 */
	public void getFile(final String filePath, final Callback<EncFSFile, Exception> callback) {
		validateAbsoluteFileName(filePath, "filePath");

		final String encryptedPath;
		try {
			encryptedPath = EncFSCrypto.encodePath(this, filePath, ROOT_PATH);
		} catch (EncFSCorruptDataException e) {
			callback.onFailure(e);
			return;
		}
		final EncFSVolume v = this;

		fileProvider.exists(encryptedPath, new Callback<Boolean, IOException>() {
			@Override
			...
			public void onSuccess(Boolean result) {
				...
				fileProvider.getFileInfo(encryptedPath, new Callback<EncFSFileInfo, IOException>() {
					@Override
					public void onFailure(IOException reason) {
						callback.onFailure(reason);
					}
					...
					@Override
					public void onSuccess(EncFSFileInfo fileInfo) {
						EncFSFileInfo decodedFileInfo = getDecodedFileInfo(filePath, fileInfo);
						EncFSFile f = new EncFSFile(v, decodedFileInfo, fileInfo);
	...
	public long getDecryptedFileLength(long encryptedFileLength) {
		...
		long headerLength = volumeConfig.getNumberOfMACBytesForEachFileBlock() + volumeConfig.getNumberOfRandomBytesInEachMACHeader();
		if (headerLength > 0) {
	 ...
	 *             File provider returned I/O error
	 */
	public void pathExists(String path, final Callback<Boolean, Exception> callback) throws EncFSCorruptDataException {
	 ...
	 */
	public static void isEncFSVolume(EncFSFileProvider fileProvider, final Callback<Boolean, Exception> callback) {
		fileProvider.exists(fileProvider.getFilesystemRootPath() + EncFSVolume.CONFIG_FILE_NAME, new Callback<Boolean, IOException>() {
			@Override
			public void onFailure(IOException reason) {
				callback.onFailure(reason);
			}
	 ...
	 */
	public boolean deletePath(String filePath, boolean recursive, EncFSProgressListener progressListener) throws EncFSCorruptDataException, IOException {
		// TODO: make async
		return false;
	...
	private boolean copyOrMovePath(String srcPath, String dstPath, PathOperation op, EncFSProgressListener progressListener) throws EncFSCorruptDataException,
	IOException {
		// TODO: make async
		return false;
		...
		// return copyOrMovePath(srcPath, combinePath(dstPath, srcFile), op,
		// progressListener);
		// } else {
	 ...
	 *             File provider returned I/O error
	 */
	public void listFilesForPath(String dirPath, final Callback<EncFSFile[], Exception> callback) {
		getFile(dirPath, new Callback<EncFSFile, Exception>() {
			@Override
			public void onFailure(Exception reason) {
				callback.onFailure(reason);
			}
			...
			@Override
			public void onSuccess(EncFSFile file) {
				file.listFiles(new Callback<EncFSFile[], Exception>() {
					@Override
					public void onFailure(Exception reason) {
						callback.onFailure(reason);
					}
	 ...
	 */
	public void openInputStreamForPath(String filePath, final Callback<EncFSInputStream, Exception> callback) {
		getFile(filePath, new Callback<EncFSFile, Exception>() {
			@Override
			public void onFailure(Exception reason) {
				callback.onFailure(reason);
			}
			...
			@Override
			public void onSuccess(EncFSFile result) {
				result.openInputStream(new Callback<EncFSInputStream, Exception>() {
					@Override
					public void onFailure(Exception reason) {
						callback.onFailure(reason);
					}
	 ...
	 */
	public EncFSOutputStream openOutputStreamForPath(String filePath, long outputLength) throws EncFSCorruptDataException, EncFSUnsupportedException,
	IOException {
