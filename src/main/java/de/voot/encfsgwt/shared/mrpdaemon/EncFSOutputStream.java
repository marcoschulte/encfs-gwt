...
public class EncFSOutputStream extends FilterOutputStream {
	 ...
	 */
	public EncFSOutputStream(EncFSVolume volume, OutputStream out, String volumePath) throws EncFSUnsupportedException, EncFSCorruptDataException {
		super(out);
		this.volume = volume;
		...
		if (config.isUseUniqueIV()) {
			...
			try {
				this.fileIv = StreamCrypto.streamDecrypt(volume, initIv, ArrayUtil.copyOf(fileHeader, fileHeader.length));
	...
	private void writeBuffer(boolean isFinal) throws IOException {
		...
		if (blockHeaderSize > 0) {
			...
			byte mac[] = EncFSCrypto.mac64(volume.getMAC(), dataBuf, blockMACLen, dataBytes - blockMACLen);
		...
		try {
			if (dataBytes == dataBuf.length) {
				...
				if (zeroBlock) {
					...
					encBuffer = BlockCrypto.blockEncrypt(volume, getBlockIV(), dataBuf);
				}
			} else {
				encBuffer = StreamCrypto.streamEncrypt(volume, getBlockIV(), dataBuf, 0, dataBytes);
			}
		} catch (IllegalBlockSizeException e) {
