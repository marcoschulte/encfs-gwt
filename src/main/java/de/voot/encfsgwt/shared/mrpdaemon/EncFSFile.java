...
public class EncFSFile {
	 ...
	 */
	public EncFSFile(EncFSVolume volume, EncFSFileInfo plainFileInfo, EncFSFileInfo cipherFileInfo) {
		this.volume = volume;
	 ...
	 * EncFSFile object.
	 */
	public void list(final Callback<String[], Exception> callback) throws IOException {
		listFiles(new Callback<EncFSFile[], Exception>() {
			@Override
			...
			@Override
			public void onSuccess(EncFSFile[] files) {
	 ...
	 */
	public void listFiles(final Callback<EncFSFile[], Exception> callback) {
		...
		volume.getFileProvider().listFiles(encDirName, new Callback<List<EncFSFileInfo>, IOException>() {
			@Override
			...
			@Override
			public void onSuccess(List<EncFSFileInfo> fileInfos) {
				...
				for (EncFSFileInfo fileInfo : fileInfos) {
					...
					if (decodedFileName != null) {
						EncFSFileInfo decEncFileInfo = EncFSFileInfo.getDecodedFileInfo(volume, dirName, decodedFileName, fileInfo);

						result.add(new EncFSFile(volume, decEncFileInfo, fileInfo));
					}
				}
				callback.onSuccess(result.toArray(new EncFSFile[result.size()]));
			}
		});
	}
	 ...
	 */
	public void openInputStream(final Callback<EncFSInputStream, Exception> callback) {
		volume.getFileProvider().openInputStream(getEncryptedPath(), new Callback<InputStream, IOException>() {
			@Override
			public void onFailure(IOException reason) {
				callback.onFailure(reason);
			}
			...
			public void onSuccess(InputStream result) {
				EncFSInputStream inputStream;
				try {
					inputStream = new EncFSInputStream(volume, result, getPath());
	 ...
	 */
	public EncFSOutputStream openOutputStream(long inputLength) throws EncFSUnsupportedException, EncFSCorruptDataException, IOException {
		return new EncFSOutputStream(volume, volume.getFileProvider().openOutputStream(getEncryptedPath(), volume.getEncryptedFileLength(inputLength)),
				getPath());
	}
