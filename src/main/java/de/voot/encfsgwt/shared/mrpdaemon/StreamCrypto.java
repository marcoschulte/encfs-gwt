...
public class StreamCrypto {
	...
	private static byte[] streamDecrypt(Cipher cipher, Mac mac, Key key, byte[] iv, byte[] ivSeed, byte[] data, int offset, int len)
		...
		EncFSCrypto.cipherInit(key, mac, Cipher.DECRYPT_MODE, cipher, iv, ivSeedPlusOne);
		...
		EncFSCrypto.cipherInit(key, mac, Cipher.DECRYPT_MODE, cipher, iv, ivSeed);
	...
	// Stream decryption implementation
	static byte[] streamDecrypt(Cipher cipher, Mac mac, Key key, byte[] iv, byte[] ivSeed, byte[] data) throws EncFSUnsupportedException,
			InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		return streamDecrypt(cipher, mac, key, iv, ivSeed, data, 0, data.length);
	}

	// Stream decryption implementation
	public static byte[] streamDecrypt(EncFSVolume volume, byte[] ivSeed, byte[] data) throws EncFSUnsupportedException, InvalidAlgorithmParameterException,
			IllegalBlockSizeException, BadPaddingException {
		Cipher streamCipher = volume.getStreamCipher();
		return streamDecrypt(streamCipher, volume.getMAC(), volume.getKey(), volume.getIV(), ivSeed, data);
	}

	// Stream decryption implementation
	public static byte[] streamDecrypt(EncFSVolume volume, byte[] ivSeed, byte[] data, int offset, int len) throws EncFSUnsupportedException,
			InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		return streamDecrypt(volume.getStreamCipher(), volume.getMAC(), volume.getKey(), volume.getIV(), ivSeed, data, offset, len);
	}
	...
	private static byte[] streamEncrypt(Cipher cipher, Mac mac, Key key, byte[] iv, byte[] ivSeed, byte[] data, int offset, int len)
		...
		byte[] encBuf = ArrayUtil.copyOfRange(data, offset, offset + len);
	...
	// Stream encryption implementation
	public static byte[] streamEncrypt(EncFSVolume volume, byte[] ivSeed, byte[] data) throws EncFSUnsupportedException, InvalidAlgorithmParameterException,
			IllegalBlockSizeException, BadPaddingException {
		return streamEncrypt(volume.getStreamCipher(), volume.getMAC(), volume.getKey(), volume.getIV(), ivSeed, data);
	}

	// Stream encryption implementation
	public static byte[] streamEncrypt(EncFSVolume volume, byte[] ivSeed, byte[] data, int offset, int len) throws EncFSUnsupportedException,
			InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		return streamEncrypt(volume.getStreamCipher(), volume.getMAC(), volume.getKey(), volume.getIV(), ivSeed, data, offset, len);
	}
	...
	public static byte[] computeChainIv(EncFSVolume volume, String volumePath) {
		...
		for (String curPath : volumePath.split(EncFSVolume.PATH_SEPARATOR)) {
			if ((curPath.length() > 0) && (!curPath.equals(EncFSVolume.PATH_SEPARATOR))) {
				byte[] encodeBytes;
				if (volume.getConfig().getFilenameAlgorithm() == EncFSFilenameEncryptionAlgorithm.BLOCK) {
					encodeBytes = EncFSCrypto.getBytesForBlockAlgorithm(curPath);
				} else {
