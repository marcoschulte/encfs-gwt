 ...
 * character. For example, the PBE mechanism defined in <a
 * href="http://www.ietf.org/rfc/rfc2898.txt"> PKCS #5</a> looks at only the low
 * order 8 bits of each character, whereas PKCS #12 looks at all 16 bits of each
 * character.
 * 
 * <p>
 ...
 * String class is immutable and there is no way to overwrite its internal value
 * when the password stored in it is no longer needed. Hence, this class
 * requests the password as a char array, so it can be overwritten when done.
 * 
 * @author Jan Luehe
 * @author Valerie Peng
 * 
...
public class PBEKeySpec implements KeySpec {
	 ...
	 * 
	 * <p>
	 * Note: <code>password</code> is cloned before it is stored in the new
	...
	public PBEKeySpec(char[] password) {
		if ((password == null) || (password.length == 0)) {
			...
			this.password = ArrayUtil.clone(password);
		}
	}
	 ...
	 * ciphers. An empty char[] is used if null is specified for
	 * <code>password</code>.
	 * 
	 * <p>
	 * Note: the <code>password</code> and <code>salt</code> are cloned before
	 * they are stored in the new <code>PBEKeySpec</code> object.
	 * 
	 * @param password
	 *            the password.
	 * @param salt
	 ...
	 *                if <code>salt</code> is null.
	 * @exception IllegalArgumentException
	 *                if <code>salt</code> is empty, i.e. 0-length,
	 *                <code>iterationCount</code> or <code>keyLength</code> is
	 *                not positive.
	 */
	public PBEKeySpec(char[] password, byte[] salt, int iterationCount, int keyLength) {
		...
		if (salt == null) {
			...
			this.salt = ArrayUtil.clone(salt);
		}
		if (iterationCount <= 0) {
	 ...
	 * specified for <code>password</code>.
	 * 
	 * <p>
	 * Note: the <code>password</code> and <code>salt</code> are cloned before
	 * they are stored in the new <code>PBEKeySpec</code> object.
	 ...
	 *                if <code>salt</code> is null.
	 * @exception IllegalArgumentException
	 *                if <code>salt</code> is empty, i.e. 0-length, or
	 ...
	 *                if password has been cleared by calling
	...
	public final char[] getPassword() {
		...
		return ArrayUtil.clone(password);
	}
